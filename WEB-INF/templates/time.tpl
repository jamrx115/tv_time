...
{foreach $project_list as $project}
	{foreach $activities_list as $activity}
		{if $project.id == $activity.project_id}
			if(typeof activities_ids[{$project.id}] == 'undefined') {
				activities_ids[{$project.id}] = new Array();
			}
			var one_activity = new Array();
			one_activity['id'] = "{$activity.id}";
			one_activity['name'] = "{$activity.name|escape:'javascript'}";
			activities_ids[{$project.id}].push(one_activity);
		{/if}
	{/foreach}
{/foreach}
...
{if (($smarty.const.TYPE_START_FINISH == $user->record_type) || ($smarty.const.TYPE_ALL == $user->record_type))}
	 ...
	 <tr>
          <td align="right">{$i18n.label.real_finish}:</td>
          <td>{$forms.timeRecordForm.real_finish.control}</td>
        </tr>
...
{if $time_records}
  ...
  {/if}
  {if (($smarty.const.TYPE_START_FINISH == $user->record_type) || ($smarty.const.TYPE_ALL == $user->record_type))}
        <td width="5%" class="tableHeader" align='right'>{$i18n.label.activity}</td>        
  ...
  {foreach $time_records as $record}
    ...
    {/if}
    {if (($smarty.const.TYPE_START_FINISH == $user->record_type) || ($smarty.const.TYPE_ALL == $user->record_type))}
        <td nowrap align='right' valign='top'>{if $record.start}{$record.activity|escape:'html'}{else}&nbsp;{/if}</td>
