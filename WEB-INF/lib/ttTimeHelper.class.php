...
class ttTimeHelper {
  ...
  static function insert($fields)
  {
    ...
    $note = $fields['note'];
    $real_finish = $fields['real_finish'];
    $billable = $fields['billable'];
    $phprojekt_id = $fields['phprojekt_id'];
    ...
    if ($duration) {
      $sql = "insert into tt_log (timestamp, user_id, date, duration, client_id, project_id, task_id, activity_id, invoice_id, comment, billable, phprojekt_id $status_f) ".
        "values ('$timestamp', $user_id, ".$mdb2->quote($date).", '$duration', ".$mdb2->quote($client).", ".$mdb2->quote($project).", ".$mdb2->quote($task).", ".$mdb2->quote($activity).", ".$mdb2->quote($invoice).", ".$mdb2->quote($note).", $billable, $phprojekt_id $status_v)";
      ...
      $sql = "insert into tt_log (timestamp, user_id, date, start, duration, client_id, project_id, task_id, activity_id, invoice_id, comment, billable, phprojekt_id $status_f) ".
        "values ('$timestamp', $user_id, ".$mdb2->quote($date).", '$start', '$duration', ".$mdb2->quote($client).", ".$mdb2->quote($project).", ".$mdb2->quote($task).", ".$mdb2->quote($activity).", ".$mdb2->quote($invoice).", ".$mdb2->quote($note).", $billable, $phprojekt_id $status_v)";
      $affected = $mdb2->exec($sql);
      if (is_a($affected, 'PEAR_Error'))
        return false;
    }
   ...
   if($real_finish) {
	$real_value = $mdb2->quote($date . ' ' . $finish);
	$sql = "UPDATE tt_project_activity_binds SET finish = $real_value WHERE project_id = ".$mdb2->quote($project)." AND task_id =".$mdb2->quote($task);
        $affected = $mdb2->exec($sql);
   }
  ...
  static function update($fields)
  {
    ...
    if ($duration) {
      $sql = "UPDATE tt_log set start = NULL, duration = '$duration', client_id = ".$mdb2->quote($client).", project_id = ".$mdb2->quote($project).", task_id = ".$mdb2->quote($task).", activity_id = ".$mdb2->quote($activity).", "."comment = ".$mdb2->quote($note).", billable = $billable, date = '$date' WHERE id = $id";
      $affected = $mdb2->exec($sql);
      if (is_a($affected, 'PEAR_Error'))
        return false;
    } else {
      ...
      $sql = "UPDATE tt_log SET start = '$start', duration = '$duration', client_id = ".$mdb2->quote($client).", project_id = ".$mdb2->quote($project).", task_id = ".$mdb2->quote($task).", activity_id = ".$mdb2->quote($activity).", "."comment = ".$mdb2->quote($note).", billable = $billable, date = '$date' WHERE id = $id";
      $affected = $mdb2->exec($sql);
      if (is_a($affected, 'PEAR_Error'))
        return false;
    }
  ...
  static function getRecord($id, $user_id) {
    ...
    $sql = "select l.id as id, l.timestamp as timestamp, TIME_FORMAT(l.start, $sql_time_format) as start,
      ...
      p.name as project_name, t.name as task_name, a.name as activity_name, l.comment, l.client_id, l.project_id, l.task_id, l.activity_id, l.invoice_id, l.billable, l.phprojekt_id, l.date
      from tt_log l
  ...
  static function getRecords($user_id, $date) {
    ...
    $left_joins = " left join tt_projects p on (l.project_id = p.id)".
      ...
      " left join tt_activities a on (l.activity_id = a.id)";
    if ($user->isPluginEnabled('cl'))
      $left_joins .= " left join tt_clients c on (l.client_id = c.id)";

    $sql = "select l.id as id, TIME_FORMAT(l.start, $sql_time_format) as start,
      TIME_FORMAT(sec_to_time(time_to_sec(l.start) + time_to_sec(l.duration)), $sql_time_format) as finish,
      TIME_FORMAT(l.duration, '%k:%i') as duration, p.name as project, t.name as task, a.name as activity, l.comment, l.billable, l.invoice_id $client_field
      from tt_log l
      $left_joins
