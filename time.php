...
require_once('api/v1/sender.php');
...
//Add variable for Real Finish and Activity
$cl_real_finish = trim($request->getParameter('real_finish'));
$cl_activity = trim($request->getParameter('activity'));
...
if (MODE_PROJECTS_AND_TASKS == $user->tracking_mode) {
  ...
  $activities_list = ttTeamHelper::getActiveActivities($user->id);
  $form->addInput(array('type'=>'combobox',
    'name'=>'activity',
    'style'=>'width: 250px;',
    'value'=>$cl_activity,
...
// Add Control for Real Finish
  $form->addInput(array('type'=>'checkbox','name'=>'real_finish','data'=>1,'value'=>$cl_real_finish));
...
if ($request->isPost()) {
  if ($request->getParameter('btn_submit')) {
    ...
    // Insert record.
    if ($err->no()) {
      $cl_real_finish = ($cl_real_finish) ? true : false;
      $api = new API_Sender($user);
      if($api->setNewTime($cl_start, $cl_finish) && $phprojekt = $api->createTaskTime($cl_activity, $cl_date, $cl_note,$cl_real_finish)) {
	   $id = ttTimeHelper::insert(array(
		...
		'note' => $cl_note,
		'real_finish' => $cl_real_finish,
		'billable' => $cl_billable,
		'phprojekt_id' => $phprojekt->ID));
...
$smarty->assign('activities_list', $activities_list);
