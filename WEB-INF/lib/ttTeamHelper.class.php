...
class ttTeamHelper {
  ...
  // The getActiveActivities returns an array of active activities for user.
  static function getActiveActivities($user_id, $all_fields = false)
  {
    $result = array();
    $mdb2 = getConnection();

    if ($all_fields)
      $sql = "select a.*, p.* from tt_activities a inner join tt_project_activity_binds p on (p.activity_id = a.id) inner join tt_activity_user_binds u on (p.activity_id = u.activity_id) where u.user_id = $user_id";
    else
      $sql = "select a.id, a.name, p.project_id from tt_activities a inner join tt_project_activity_binds p on (p.activity_id = a.id) inner join tt_activity_user_binds u on (p.activity_id = u.activity_id) where u.user_id = $user_id";
